...
def cleanStop(sw):
    ...
    csw.extend(esw)
    return csw
#   The following function utilizes beautiful soup to obtain the justifications
#   and put them into a string separated by next line.
def bsMeth(s):
    # Use beautiful Soup to separate the justifications
    with open(s) as fp:
...
#   The following function receives the justifications and separates them. It
#   creates a dictionary-matrix with the following structure:
#   Justifications-Number-Word
...
#   Select a decent size of vectors to work with
TSU_matrix, Lic_matrix, Maestria_matrix  = get3NVectors(50,TSU_matrix, Lic_matrix, Maestria_matrix)
...
#   We obtain the vocabulary.
voc = obtainVoc3(TSU_matrix, Lic_matrix, Maestria_matrix)
...
#eightyPOneMaestria, eightyMOneMaestria = comData(eightyConMaestria, eightyConDoctorado)
...
tM, ty = allTogetherNow(twentyPOneTSU, twentyMOneTSU, twentyPOneLic, twentyMOneLic)
TM, Ty = allTogetherNow(eightyPOneTSU, eightyMOneTSU, eightyPOneLic, eightyMOneLic)

# print(M)
# print(y)
print("Length training matrix:", len(TM))
print("Length classification vector for training matrix:", len(Ty))
print("Length test matrix:", len(tM))
print("Length classification vector for test matrix:", len(ty))
...
random_list, position_list = obtainRandomVectors3(eightyConTSU, eightyConLic, eightyConMaestria)
#Obtain the matrix with the test vectors to measure our evaluator.
evaluator_matrix, y = testEvaluatorMatrix3(twentyConTSU, twentyConLic, twentyConMaestria)
...
#acc_random = getJustificationEvaluation(evaluator_matrix, y, random_list, svclassifier, justificacionTSU, justificacionLic, justificacionMaestria)
centroid_list = obtainCentroids3(eightyConTSU, eightyConLic, eightyConMaestria)
...
acc_random = getJustificationEvaluation(evaluator_matrix, y, random_list, svclassifier, justificacionTSU, justificacionLic, justificacionMaestria)
print("Position of random vectors", position_list)
getRandomJustifications(position_list, justificacionTSU, justificacionLic, justificacionMaestria)
acc_random = randomEvaluator(evaluator_matrix, y, random_list, svclassifier)
print("Accuracy for random evaluator: ", acc_random)
